...
def calcgcseavg(resultMapping):
  ...
  percentageDifference = ((float(average)-resultMappingReturned)/resultMappingReturned)*100 
  ...
  for i in range(0, len(gcses)):
    ...
    if tempCheck.isalpha() == False:
      ...
      for i in range(len(gcsesList)):
        resultMapping = sum(map(lambda x: dictOfScores.get(x, 0), gcsesList)) #maps user input in gcsesList into a dictionary and returns a numerical value (total sum of all key:value pairs given by user)
      ...
      try:
        ...
        try:
          ...
          userFinalAverage =  (float(UserGCSEScore) * 1) + (float(UserAlvlScore) * 6) + (MATScore * 3)/10
          print(f"userfinal{userFinalAverage}")
          
          percentageDifference = ((float(averageCambridgeScore)-float(userFinalAverage))/userFinalAverage)*100
          ...
          await ctx.send(f"You have a {round(percentageDifference, 2)}% chance of getting into an Oxbridge university")
